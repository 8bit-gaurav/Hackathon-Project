...
# 1. Initialize memory bank globally
global_session_svc = InMemorySessionService()

# 2. Initialize Runner globally
global_runner = Runner(
    app_name="medicare_os", 
    agent=root_agent, 
    session_service=global_session_svc
)

# Flag to prevent redundant session creation
session_initialized = False

async def process_request(user_input, history):
    ...
    if not session_initialized:
        try:
            await global_session_svc.create_session(
                app_name="medicare_os",
                user_id="local_dev",
    ...
    try:
        ...
        async for event in events:
            # FIX: Only extract and return if the event contains actual text.
            # This safely ignores the background tool calls while it searches the database.
            if hasattr(event, 'content') and event.content and event.content.parts:
                text_content = event.content.parts[0].text
                if text_content: # If text exists, it's not a tool call!
                    return text_content.strip()
                
        return "I'm checking that for you, please hold on."
